...
public class Game {
    ...
    private void singleRound(Pawn[][] board, Player activePlayer){
        ...
        while (!validator.isMoveValid(board, pawnToMove.getCoordinateX(), pawnToMove.getCoordinateY(),
                moveCoordinates[0], moveCoordinates[1])){
            System.out.println(ConsoleColors.RED_BOLD_BRIGHT + "You cannot move there! " +
                    "Choose the correct coordinate!" + ConsoleColors.RESET);
            moveCoordinates = userHandler.getMoveCoordinates(board);
        }
